...
void main(void)
{
	...
	/* Enable Periphral 0 on APB2 Bus (AFIO) */
	RCC_voidEnableClock(RCC_APB2, 0);
/******************************************************************************************/
	/* A0 Input pull up */
	GPIO_voidSetPinDirection(GPIOA_ID, PIN0, INPUT_PULL_UP_DOWN);
	GPIO_voidSetPinValue(GPIOA_ID, PIN0, LOGIC_HIGH);
	...
	MEXTI_voidEXTI0SetCallBack(ToggleLed);
	...
	/* Enable A0 as EXTI working in falling edge */
	MEXTI_voidSetSignalLatch(EXTI_LINE0, FALLING_EDGE);

	/* Set AFIO pin (A0) */
	MAFIO_voidSetEXTI(AFIO_LINE0, AFIO_PORT_MAP_A);

/******************************************************************************************/
	/* Set priority of EXTI0 */
	//MNVIC_voidSetPriority(6, 0, 3);	/* EXTI0 GROUP 0 SUB 3 */
